...
public class Util {
    ...
    public static byte[] longToBytes(long num) {
        ...
        for (int i = 0; i < byteLen; i++) {
            bytes[i] = (byte) (0xff & (num >> (i * byteLen)));
        }
    ...
    public static byte[] byteConvert32Bytes(BigInteger n) {
        ...
        if (n.toByteArray().length == Constants.THIRTY_THREE) {
            tmpd = new byte[32];
            System.arraycopy(n.toByteArray(), 1, tmpd, 0, 32);
        } else if (n.toByteArray().length == Constants.THIRTY_TWO) {
            tmpd = n.toByteArray();
        } else {
            tmpd = new byte[32];
            for (int i = 0; i < Constants.THIRTY_TWO - n.toByteArray().length; i++) {
                tmpd[i] = 0;
            }
    ...
    public static String algorismToHexString(int algorism, int maxLength) {
        ...
        if (result.length() % Constants.TWO == 1) {
            result = "0" + result;
        }
    ...
    public static String algorismToHexString(int algorism) {
        String result = Integer.toHexString(algorism);
        if (result.length() % Constants.TWO != 0) {
            result = "0" + result;
        }
     ...
     *
     * @param str        要转换的字符串
     * @param defaultInt 如果出现异常,默认返回的数字
     * @param radix      要转换的字符串是什么进制的,如16 8 10.
     * @return int 转换后的数字
     */
    public static int parseToInt(String str, int defaultInt, int radix) {
        try {
     ...
     * @return int 转换后的数字
     */
    public static int parseToInt(String str, int defaultInt) {
        try {
    ...
    public static byte[] subByte(byte[] input, int startIndex, int length) {
        if (input.length < startIndex + length) {
            throw new IllegalArgumentException("sub array index range out of input array");
        }
